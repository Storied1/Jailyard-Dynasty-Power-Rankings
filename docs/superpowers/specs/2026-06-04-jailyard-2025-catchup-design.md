...
6. **`team-profiles.json` field-preserving merge:** update prose fields (rank, tier, roast, blurb, preseasonEssay); preserve structured fields (initials, ranks{}, keyPlayers, draftPicks, weeklyPoints, scheduleRank, needs, championshipHistory, owner spellings — incl. fixing the `kharlo w`/`kharlow` mismatch). Then re-run `extract_week_data.py --all --pretty` (temporally safe: preseason precedes all weeks) + re-verify weeks 1-6.
