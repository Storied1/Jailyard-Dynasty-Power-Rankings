...
- **After every push, watch CI:** `gh run watch $(gh run list --limit 1 --json databaseId --jq '.[0].databaseId') --exit-status` — CI sat red for 2 days unnoticed (2026-07-07→09) because nobody looked; a push isn't done until the run is green.
