...
| `week1.html`          | Weekly column (weeks regenerate one at a time through the rewrite pipeline)  |
...
- `content/preseason-2025/preseason_chat_context.json` (sanitized preseason chat DATA — kept) + `content/review-log.jsonl` (shared append-only JSONL editorial log, one line per `/edit-week`+`/edit-preseason` pass). The old `preseason_content.json` writer output was deleted 2026-08-10 under THE COPY LAW.
...
- **Add chart:** Canvas 2D pattern from `power-rankings.html`, always handle `devicePixelRatio`
