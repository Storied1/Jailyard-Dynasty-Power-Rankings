...
const LEAGUE_CONFIG = {
  ...
  pages: [
    ...
    { label: 'Week 1', href: 'week1.html', group: 'columns',
      subtitle: 'Opening week chaos sets the tone' },
    { label: 'Week 2', href: 'week2.html', group: 'columns',
      subtitle: 'Early risers and first-week frauds' },
    { label: 'Week 3', href: 'week3.html', group: 'columns',
      subtitle: 'The cream separates from the chaos' },
    { label: 'Week 4', href: 'week4.html', group: 'columns',
...
function renderNav() {
  ...
  var corePages = cfg.pages.filter(function(p) { return !p.group; });
  var columns = cfg.pages.filter(function(p) { return p.group === 'columns'; });
...
'.light .latest-pill{background:rgba(247,249,252,0.9);}'
  ...
  // ── Build top nav HTML ──
  var pillsHtml = corePages.map(function(p) {
  ...
  for (var w = 1; w <= cfg.totalWeeks; w++) {
    ...
    for (var ci = 0; ci < columns.length; ci++) {
      if (columns[ci].label === 'Week ' + w) { col = columns[ci]; break; }
    }
    if (col) {
      ...
      nodesHtml += '<a href="' + col.href + '" class="' + cls + '" data-week="' + w + '" data-subtitle="' + (col.subtitle || '') + '">' + w + '</a>';
    } else {
      nodesHtml += '<span class="strip-node ghost" data-week="' + w + '"></span>';
    }
    if (w < cfg.totalWeeks) nodesHtml += '<span class="strip-spacer"></span>';
  }
  var fillPct = cfg.totalWeeks > 1 ? ((latestWeek - 1) / (cfg.totalWeeks - 1) * 100) : 0;

  // ── Build floating pill ──
  var pillHtml = '';
  if (latestCol && !isOnLatest) {
    pillHtml = '<a href="' + latestCol.href + '" class="latest-pill"><span class="pill-dot"></span>' + latestCol.label + '</a>';
  }

  // ── Assemble (trusted config data only) ──
  target.insertAdjacentHTML('beforeend',
'<nav class="site-topnav" id="siteTopnav">' +
'  <a href="index.html" class="nav-brand">' + cfg.name + '</a>' +
'  <div class="nav-pills" id="navPills">' + pillsHtml + '</div>' +
'  <div class="nav-right">' +
...
'    <div class="strip-line-fill" style="width:' + fillPct + '%"></div>' +
    nodesHtml +
'  </div>' +
'</div>' +
pillHtml);
  ...
  document.addEventListener('click', function(e) {
    if (!e.target.closest('.strip-node') && !e.target.closest('.strip-tooltip')) hideTooltip();
  });
  ...
  if (window.innerWidth <= 768 && latestCol) {
    ...
    var latestNode = strip.querySelector('.strip-node.latest') || strip.querySelector('.strip-node.active');
    if (latestNode) {
